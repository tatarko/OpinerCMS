...
$query = @mysql_query ("SELECT DISTINCT DATE_FORMAT(`kedy`, '%Y-%m-%d') FROM `{$prefix}_hits` WHERE `kedy` > SUBDATE('{$_SESSION['day']}', 3) AND `kedy` < ADDDATE('{$_SESSION['day']}', 4)");
while ($tab = @mysql_fetch_row ($query)) {
	$kolko = @mysql_fetch_row (@mysql_query ("SELECT COUNT(*), COUNT(DISTINCT ip) FROM {$prefix}_hits WHERE DATE_FORMAT(kedy,'%Y-%m-%d') = '$tab[0]'"));
	$kolk2 = @mysql_fetch_row (@mysql_query ("SELECT COUNT(*) FROM {$prefix}_visits WHERE DATE_FORMAT(kedy,'%Y-%m-%d') = '$tab[0]'"));
	$array[] = array (
		'day'		=> substr ($tab[0], 8, 2),
		'hits'		=> $kolko[0],
		'visits'	=> $kolk2[0],
		'uip'		=> $kolko[1],
	);
